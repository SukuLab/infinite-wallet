  ...
  <section class="import-account">
    ...
    <section class="container">
      <section class="details">
        <Dropdown
          :selected="importingNetwork"
          :options="['Mainnet', 'Testnet', 'Previewnet']"
          v-on:selected="(x) => (importingNetwork = x)"
        />
        <input
          type="password"
          class="square"
          v-model="mnemonicOrPrivateKey"
          placeholder="Enter your phrase or private key"
        />
        <input
          v-if="importingNetwork !== 'Testnet'"
          class="square"
          v-model="manualAccount"
          placeholder="Enter your account ID"
        />
        <figure class="account-length" v-if="importingAccounts.length">
          Importing <b>{{ importingAccounts.length }}</b> account{{
            importingAccounts.length === 1 ? "" : "s"
          }}
          for {{ importingNetwork }}
        </figure>
        <figure class="account-length" v-if="!importingAccounts.length">
          No accounts found for {{ importingNetwork }}
        </figure>
      </section>
      ...
      <section class="actions">
        ...
        <div>
          On second thought,
          <span @click="$emit('done', false)">nevermind.</span>
        </div>
      </section>
    </section>
  </section>
...
export default {
  ...
  methods: {
    ...
    async checkAccounts(key) {
      ...
      this.importingAccounts = await ApiService.getAccountsFromPublicKey(
        this.importingNetwork,
        key.publicKey
      );
    },
    async importAccount() {
      ...
      if (this.showPasswordField) {
        const restored = await InternalMessage.payload(
          InternalMessageTypes.RESTORE,
          {
      ...
      let accounts = [];
      if (this.importingNetwork === "Testnet")
        accounts = await ApiService.getAccountsFromPublicKey(
          "Testnet",
          key.publicKey
        );
      ...
      const inserted = await InternalMessage.payload(
        InternalMessageTypes.INSERT_ACCOUNTS,
        { accounts, privateKey: key.toString() }
      ).send();
      if (inserted) {
