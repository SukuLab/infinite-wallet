<template>
  <section class="swipe-container">
    ...
    <figure
      class="deny"
      :class="{ 'no-hover': noCancel || swipePercentage > PERCENTAGE_TO_SWIPE }"
      @click="deny"
    >
      <i
        v-if="!noCancel"
        :style="{
          transform: `translateX(${clamp(-60, 0, 1 - swipePercentage * 40)}px)`,
          opacity: 1 - swipePercentage - parseFloat(swipePercentage * 0.5)
        }"
        class="fas fa-times"
      ></i>
      <i
        :style="{
          transform: `translateX(-${50 -
            clamp(0, 50, swipePercentage * 80)}px)`,
    ...
    <figure class="swiper" ref="swiper">
      <span
        :style="{
          color: `rgba(${clampedTextColor}, ${clampedTextColor}, ${clampedTextColor}, ${clamp(
            0.5,
            1,
            swipePercentage
          )})`
        }"
        >{{ text || "Slide to approve" }}</span
      >
      <figure
        :class="{
          'quick-move': dragging,
          pop: !dragging && swipePercentage > PERCENTAGE_TO_SWIPE
        }"
        :style="{ left: `${sliderLeft + 2}px` }"
        ref="slider"
        class="slider"
        @mousedown="dragStart"
      >
  ...
  methods: {
    ...
    async dragStop(event) {
      ...
      if (this.swipePercentage > PERCENTAGE_TO_SWIPE) {
        ...
        this.sliderLeft =
          this.swiperEnd - this.$refs.slider.getBoundingClientRect().width - 31;
...
.swipe-container {
  ...
  .swiper {
    ...
    text-align: center;
    margin-bottom: 5px;
    box-shadow: 10px 0 15px rgb(0 0 0 / 10%), inset 0 0 0 2px white,
      inset 0 0 31px 10px rgb(0 0 0 / 8%);
