<template>
  <section class="navbar" :class="{ 'account-selector': accountSelector }">
    <section
      ...
      v-if="iv"
      :class="{ show: accountSelector }"
    >
      <ImportAccount
        v-if="importingAccount"
        v-on:done="importingAccount = false"
      />
      <ViewPrivateKey
        v-if="showingPrivateKey"
        :account="showingPrivateKey"
        v-on:done="showingPrivateKey = null"
      />
      <section class="accounts">
        <section @click="importingAccount = true" class="account import-bar">
          <figure class="identicon" @click="accountSelector = !accountSelector">
            <img
              :src="`data:image/png;base64, ${getIdenticon('Import account')}`"
            />
          </figure>
          <section class="account-info">
            <figure class="id">Import an account</figure>
            <figure class="network">
              You can select this option if you want to add a new account to
              your wallet, however it must already exist.
            </figure>
          </section>
        </section>
      </section>
      <section class="accounts">
        <section
          :class="{
            active: activeAccount && activeAccount.id === account.id,
            pressed: accountOptions === account.id
          }"
          @click="openAccountActions(account)"
          :key="`${account.id}:${account.network}`"
          class="account"
          v-for="account in iv.keychain.accounts"
        >
          <figure class="identicon" @click="selectAccount(account)">
            <img
              :src="
                `data:image/png;base64, ${getIdenticon(
          ...
          <section style="width:100%; position:relative; flex:1;">
            <transition name="slide-left" mode="out-in">
              <section
                key="account-info"
                class="account-info"
                v-if="accountOptions !== account.id"
              >
            ...
            <transition name="slide-left" mode="out-in">
              <section
                ...
                v-if="accountOptions === account.id && !removingAccount"
              >
                <figure
                  v-if="
                    iv.keychain.accounts.length > 1 &&
                      activeAccount &&
                ...
                <figure
                  class="action"
                  @click="showingPrivateKey = account"
                  v-tooltip.left="{ offset: 5, content: 'View private key' }"
                >
                  <i class="fas fa-key"></i>
                </figure>
                <figure
                  v-if="activeAccount && activeAccount.id !== account.id"
                  class="action"
                  @click="selectAccount(account)"
                  v-tooltip.left="{ offset: 5, content: 'Select' }"
                >
                  <i class="fas fa-check"></i>
                </figure>
              </section>
              <section
                key="removing-account"
                class="account-actions"
                v-if="accountOptions === account.id && removingAccount"
              >
                <figure
                  class="action warn"
                  @click="cancelAccountRemoval"
                  style="flex:0 0 auto; width:auto;"
                >
                  <span>Cancel ({{ removeAccountTimerCounter }})</span>
                </figure>
              </section>
            </transition>
          </section>
    ...
    <section class="nav-container">
      ...
      <figure class="info" v-if="hasAccount">
        <section class="details">
          <figure class="account" v-if="activeAccount">
          ...
          <figure
            class="balance"
            @click="reloadBalance"
            v-tooltip.bottom="'Reload Balance'"
            v-if="hbarBalance"
          >
            <AnimatedNumber :number="hbarBalance" :decimals="8" /> <b>HBAR</b>
          </figure>
          <figure class="balance" v-if="!hbarBalance && activeAccount">
            <i class="fa fa-spinner fa-spin"></i>
          </figure>
          <figure class="balance" v-if="!hbarBalance && !activeAccount">
            <!-- placeholder, don't remove -->
          </figure>
          <figure class="network" v-if="activeAccount">
            <b>{{ activeAccount.network }}</b>
          </figure>
        </section>
        <figure class="identicon" @click="accountSelector = !accountSelector">
          <img
            :src="
              `data:image/png;base64, ${getIdenticon(
                activeAccount.name + activeAccount.network
              )}`
            "
          />
        </figure>
      ...
      <figure class="info" @click="accountSelector = !accountSelector" v-else>
        ...
        <figure class="identicon">
          <img
            :src="`data:image/png;base64, ${getIdenticon('No account yet')}`"
          />
        </figure>
      </figure>
    </section>
...
export default {
  ...
  methods: {
    ...
    async completeRemoveAccount() {
      const account = this.iv.keychain.accounts.find(
        (x) => x.id === this.removingAccount
      );
      const deleted = await InternalMessage.payload(
        InternalMessageTypes.DELETE_ACCOUNTS,
        [account]
      ).send();
    ...
    ...mapActions(["setBalance", "setActiveAccount"])
  },
  watch: {
    activeAccount() {
      this.getAccountInfo();
...
.navbar {
  ...
  .identicon {
    ...
    box-shadow: inset 0 0 10px rgba(0, 0, 0, 0.1),
      inset 0 -2px 8px rgba(0, 0, 0, 0.05), inset 0 0 20px rgba(0, 0, 0, 0.08);
    width: 40px;
    height: 40px;
    flex: 0 0 auto;
    ...
    &:hover {
      box-shadow: inset 0 0 0 rgba(0, 0, 0, 0), inset 0 0 0 rgba(0, 0, 0, 0),
        inset 0 0 0 rgba(0, 0, 0, 0), 0 6px 13px rgba(0, 0, 0, 0.1),
        0 12px 34px rgba(0, 0, 0, 0.05);
    ...
    &:active {
      box-shadow: inset 0 0 0 rgba(0, 0, 0, 0), inset 0 0 0 rgba(0, 0, 0, 0),
        inset 0 0 0 rgba(0, 0, 0, 0), 0 1px 2px rgba(0, 0, 0, 0.12),
        0 3px 5px rgba(0, 0, 0, 0.07);
