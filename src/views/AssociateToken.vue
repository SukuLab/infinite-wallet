    ...
    <transition name="pop-between" mode="out-in">
      <section key="1" class="container" v-if="status === STATUS.DETAILS">
        <section style="width:100%; display:flex; align-items: center;">
          ...
          <button
            style="width:auto; flex:0 0 auto; padding:0 20px; margin-bottom:5px; margin-left:5px"
            class="secondary"
            @click="getTokenInfo"
            v-tooltip="'Load token info <br />(costs $0.0001)'"
          >
        ...
        <section class="details" v-if="!tokenInfo">
          Accepting / associating a token with your account means you are
          allowing people to send it to you.
          <br />
          <br />
      ...
      <section key="3" class="container centered" v-if="status === STATUS.SENT">
        <section class="transfer-panel">
          ...
          <figure class="text">
            You allowed accepted <b>{{ tokenId }}</b> tokens for
    ...
    async getTokenInfo() {
      ...
      if (query) {
        ...
        let totalSupply = parseFloat(
          parseFloat(query.totalSupply.toString()) /
            parseInt(query.decimals === 0 ? 1 : query.decimals)
        );
        this.tokenInfo = {
          name: query.name,
