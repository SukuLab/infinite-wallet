<template>
  <section class="prompt-signature">
    ...
    <section class="details">
      ...
      <section class="transfer-hbar" v-if="isTransfer">
        <figure class="transaction-type">
          Token Transfer{{ transferToCount > 1 ? "s" : "" }}
          <span v-if="transferToCount > 1">{{ transferToCount }}</span>
        </figure>

        <!-- HBAR -->
        <section v-if="hasHbarTransfers">
          <figure class="transfer-total">
            {{ totalHbarFrom }} <b>HBAR</b>
          </figure>
          <figure
            class="transfer-details"
            :key="n"
            v-for="(x, n) in hbarTransfersTo"
          >
            +{{
              parseFloat(parseFloat(x.amount.toString()) / 100000000).toFixed(8)
            }}
            <b>HBAR</b> to
            <b>{{ accountIdToString(x.accountID) }}</b>
          </figure>
        </section>

        <!-- TOKENS -->
        <figure
          class="transfer-total"
          :key="n"
          v-for="(x, n) in tokenTransfersFrom"
        >
          {{ x.amount }} <b>{{ x.token.symbol || x.token.id }}</b>
        </figure>
        <figure
          class="transfer-details"
          :key="n"
          v-for="(x, n) in tokenTransfersTo"
        >
          +{{ x.amount }} <b>{{ x.token.symbol || x.token.id }}</b> to
          <span>{{ x.account }}</span>
        </figure>
      </section>
        ...
        <section class="key-values">
          <figure class="param" :key="key" v-for="(value, key) in params">
            ...
            <figure class="value" v-else>
              <VueJsonViewer theme="jstheme" :value="value" />
            </figure>
          </figure>
        </section>
      </section>
    ...
    <section class="actions">
      <figure class="fee">
        <span>Fee</span> {{ transactionFee }} <b>HBAR</b>
      </figure>
...
export default {
  ...
  async mounted() {
    this.getTokenMeta();
    if (!this.isTransfer)
      this.formatKeyValues(this.prompt.data.details[this.transactionType]);
  },
  data() {
    return {};
  },
  computed: {
    ...
    transactionFee() {
      return parseFloat(
        parseFloat(this.prompt.data.details.transactionFee.toString()) /
          100000000
      ).toFixed(8);
    },
    hasHbarTransfers() {
      return !!this.prompt.data.details.cryptoTransfer.transfers.accountAmounts
        .length;
    },
    hbarTransfersFrom() {
      return this.prompt.data.details.cryptoTransfer.transfers.accountAmounts.filter(
        (x) => {
          return (
            this.accountIdToString(x.accountID) ===
            this.prompt.data.account.name
          );
        }
      );
    },
    totalHbarFrom() {
      const total = this.hbarTransfersFrom.reduce(
        (acc, x) => parseFloat(acc + parseFloat(x.amount.toString())),
        0
      );
    ...
    tokenTransfers() {
      return this.prompt.data.details.cryptoTransfer.tokenTransfers.reduce(
        (acc, x) => {
          x.transfers.map((transfer) => {
            ...
            acc.push({
              ...
              account: this.accountIdToString(transfer.accountID),
  ...
  methods: {
      ...
      for (let property in obj) {
        if (obj.hasOwnProperty(property)) {
          ...
          if (typeof obj[property] === "object") {
            if (this.isLong(obj[property]))
              obj[property] = obj[property].toString();
            else if (this.isAccountIdString(obj[property]))
              obj[property] = this.accountIdToString(obj[property]);
            else if (this.isTokenIdString(obj[property]))
              obj[property] = this.tokenIdToString(obj[property]);
            else if (obj[property].hasOwnProperty("ed25519")) {
              obj[property] = PublicKey.fromBytes(
                obj[property].ed25519
              ).toString();
    ...
    isLong(obj) {
      return (
        obj.hasOwnProperty("low") &&
        obj.hasOwnProperty("high") &&
        obj.hasOwnProperty("unsigned")
      );
    },
    isAccountIdString(obj) {
      return (
        obj.hasOwnProperty("shardNum") &&
        obj.hasOwnProperty("realmNum") &&
        obj.hasOwnProperty("accountNum")
      );
    },
    isTokenIdString(obj) {
      return (
