  ...
  <div class="login">
    ...
    <transition mode="out-in">
      ...
      <section class="details" v-if="state === STATES.DESTROY">
        There is no way to recover your password itself, but if you have your 24
        words or a private key you can just re-create your IV.
        <br />
        <br />
    ...
    <transition mode="out-in">
      <section class="actions" v-if="state === STATES.LOGIN">
        ...
        <div>
          Did you
          <span @click="state = STATES.DESTROY">lose your password?</span>
        </div>
      </section>
      ...
      <section class="actions" v-if="state === STATES.WIZARD_ONE">
        ...
        <div>
          Make a mistake?
          <span @click="setState(STATES.CREATE_NEW_INSTANCE)">Go back</span>
        </div>
      </section>
      ...
      <section class="actions" v-if="state === STATES.WIZARD_THREE"></section>
    </transition>
  </div>
</template>
...
export default {
  ...
  async mounted() {
    const hasData = await InternalMessage.signal(
      InternalMessageTypes.HAS_DATA
    ).send();
  ...
  methods: {
    ...
    async unlock() {
      const unlocked = await InternalMessage.payload(
        InternalMessageTypes.UNLOCK,
        this.password
      ).send();
    ...
    async createInstance() {
      const mnemonic = await InternalMessage.payload(
        InternalMessageTypes.SETUP,
        {
          state: this.state,
          data: this.password,
