    ...
    <section class="details">
      ...
      <section v-if="availableAccounts.length > 1">
        ...
        <Dropdown
          ...
          :options="availableAccounts.map((x) => x.name)"
          v-on:selected="(x) => (selectedAccount = x)"
        />
      </section>
      <figure class="disclaimer">
        If you approve this login request then this website will be able to see
        your account details, and request transaction signatures from you.
      </figure>
    </section>
...
export default {
  async mounted() {
    this.selectedAccount = this.availableAccounts.length
      ? this.availableAccounts[0].name
      : null;
  },
  data() {
    return {
  ...
  computed: {
    ...mapState(["prompt"]),
    availableAccounts() {
      return this.prompt.iv.keychain.accounts.filter(
        (x) => x.network === this.prompt.data.network
      );
    }
  },
  methods: {
