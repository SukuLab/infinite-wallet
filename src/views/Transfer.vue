  ...
  <section class="transfer" @mouseup="endErase">
    ...
    <transition name="pop-between" mode="out-in">
      <section key="1" class="container" v-if="status === STATUS.DETAILS">
        <Dropdown
          :selected="tokenId"
          :options="['HBAR'].concat(tokenBalances.map((x) => x.id))"
          v-on:selected="(x) => (tokenId = x)"
        />
        <input
          class="square"
          v-model="recipient"
        ...
        <section style="width:100%;">
          ...
          <section class="numpad">
            <figure
              @click="clickNumpad(x)"
              :key="x"
              v-for="x in [1, 2, 3, 4, 5, 6, 7, 8, 9, '.', 0]"
            >
              {{ x }}
            </figure>
            <figure @mousedown="startErase">
...
export default {
        ...
        if (this.tokenId === "HBAR") {
          tx = new TransferTransaction()
            .addHbarTransfer(
              ...
              Hbar.fromString(amount, HbarUnit.Hbar).negated()
            ) //
            .addHbarTransfer(
              this.recipient,
          ...
          const correctedAmount =
            parseFloat(this.amount) ** parseInt(meta.decimals);
          tx = new TransferTransaction()
            .addTokenTransfer(
              this.tokenId,
              this.activeAccount.name,
  ...
  watch: {
    amount() {
      const chars = [1, 2, 3, 4, 5, 6, 7, 8, 9, 0, "."].map((x) =>
        x.toString()
      );
